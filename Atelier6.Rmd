...
```{r}
Infarct <- read.csv2("C:/Users/thoma/Desktop/Atelier6-master/Infarct.csv")
names(Infarct)
```
...
Commentaire: comme la moyenne et la médiane sont très proche on peut dire que la taille suit une loi normale.
...
Commentaire:la moyenne et la mediane sont quasiment égale donc le poids suit une loi normale
...
Commentaire: on peut donc voir que 33% des femmes qui ont pris des contraceptifs oraux ont fait des infarctus.
...
```
 ...
 Commentaire: on peut donc dire que 44,5% des femmes qui ont fait un infartus prennaient un contraceptif orale.
...
Commentaire:on remarque que IMC1=IMC donc l'IMC de la base de donnée est bien calculé.
...
Commentaire:on peut voir que le fait de consommé du tabac ne change pas les risque de faire des infarctus
...
```
  ...
  Commentaire:on remarque que les femmes qui ont eu un infarctus on un poids plus élevé que celle qui nous pas eu d'infarctus
...
```
  ...
  Commentaire:on peut voir que la taille ne joue pas un role dans le fait d'avoir un infarctus ou non.